...
func (p *ParentController) handleChildMessage(msgBuf []byte) {
	...
	case ipcgen.MessageTypeSTATUS_RESPONSE:
		...
		if msg.Payload(payloadTable) {
			statusPayload := new(ipcgen.StatusResponsePayload)
			statusPayload.Init(payloadTable.Bytes, payloadTable.Pos)
			
			status := statusPayload.Status()
			errorMsg := string(statusPayload.ErrorMessage())
			additionalInfo := string(statusPayload.AdditionalInfo())
			
			p.logger.Printf("Status: %s, Message: %s, Info: %s", 
				ipcgen.EnumNamesConnectionStatus[status], errorMsg, additionalInfo)
			
			// Update connection state
			if status == ipcgen.ConnectionStatusConnected || 
			   status == ipcgen.ConnectionStatusReconnected {
				p.mu.Lock()
				p.isConnected = true
				p.mu.Unlock()
			} else if status == ipcgen.ConnectionStatusDisconnected ||
			          status == ipcgen.ConnectionStatusConnectionLost ||
			          status == ipcgen.ConnectionStatusFailed {
				p.mu.Lock()
				p.isConnected = false
				p.mu.Unlock()
			}
	...
	case ipcgen.MessageTypeLOG_RESPONSE:
		payloadTable := new(flatbuffers.Table)
		if msg.Payload(payloadTable) {
			logPayload := new(ipcgen.LogResponsePayload)
			logPayload.Init(payloadTable.Bytes, payloadTable.Pos)
			
			level := logPayload.Level()
			message := string(logPayload.Message())
			
			p.logger.Printf("[child-log][%s] %s", 
				ipcgen.EnumNamesLogLevel[level], message)
		}
...
func (p *ParentController) StreamVideo(stopChan <-chan struct{}) {
	...
	frameSize := p.videoWidth * p.videoHeight * 3 / 2
	frameBuf := make([]byte, frameSize)
